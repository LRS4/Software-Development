// Resizes a BMP file, given factor (f)
...
int main(int argc, char *argv[])
{
    ...
    new_bi.biWidth *= f;   // bi.biWidth = bi.biWidth * f
    new_bi.biHeight *= f;  // bi.biHeight = bi.Height * f
    ...
    {
        for (int repeat = 0; repeat < f; repeat++)
        {
            ...
            if (repeat < f-1)
            {
                // rewind the current scanline to n-1 times
